...
/// [SpeakButton]の3状態。
///
/// 押してから音が鳴り始めるまでには端末の音声エンジンの準備時間があるので、
/// 「押した／押していない」の2状態だと反応が無いように見える。準備中を
/// 挟んで、押したことが必ず見た目に出るようにする。
enum SpeakButtonState {
  /// 待機中（押すと読み上げが始まる）
  idle,

  /// 押した直後。音声の準備待ちで、まだ鳴っていない
  preparing,
...
///
/// 通常はオレンジ枠＋白地、準備中・読み上げ中（[state]）はオレンジ薄背景に
/// 変わり、アイコンとラベルが「準備中」→「停止」と切り替わる
/// （読み上げ中にもう一度押すと止められる）。
///
...
class SpeakButton extends StatelessWidget {
  ...
  /// タップ時のコールバック（読み上げ開始／準備中・読み上げ中なら停止）
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              // 準備中は「押したのに無音」の間を埋めるため、アイコンの代わりに
              ...
              Text(
                switch (state) {
                  ...
                  SpeakButtonState.preparing => '準備中',
