...
- **口頭英作文**: 日本語文を見て制限時間内に英語で発話 → 音声認識で文字起こし → Gemini が採点・添削。音声で回答した場合は録音を使った**発音評価**（総合スコア・単語ごとの指摘・アドバイス）も並列で行う
...
| 口頭英作文 | テーマ（日常/ビジネス/旅行）×レベル（700/800）でデッキ選択 → 出題一覧 → 発話＆採点（添削＋発音評価） → 添削表示 → まとめ画面（平均スコア・発音平均） |
...
- **Android / iOS**: マイク権限が必要です（Android: `RECORD_AUDIO`、iOS: `NSMicrophoneUsageDescription`。マニフェストには設定済みですが、初回起動時にOSの権限ダイアログが表示されるので許可してください）。権限を拒否した場合でも、テキスト入力欄への手入力でトレーニングを続行できます。
- **Web**: `flutter run -d chrome` で動作します。録音はメモリ上のPCMストリームで扱いファイルI/Oを使わないため、ブラウザでも同じコードパスで動作します。ビルド確認は `flutter build web` で通ることを確認しています。
...
- 音声入力: `record` で録音（PCM16 16kHz mono）→ WAV化 → Gemini文字起こし。`SpeechInputService` で抽象化（テストではフェイクに差し替え）
