...
| 口頭作文 | テーマ（日常/ビジネス/旅行）×レベル（英語 700/800、中国語 HSK3/4）でデッキ選択 → 出題一覧 → 発話＆採点 → 添削表示（問題文・修正版/模範解答の読み上げ付き。読み上げは Cloud TTS の WaveNet 音声） → まとめ画面（平均スコア、問ごとの結果、APIトークン使用量と概算コスト） |
...
保存したAPIキーは端末の secure storage（`flutter_secure_storage`）に暗号化して保存され、Google のAPIへのリクエスト以外の目的で外部に送信されることはありません。キーは設定画面からいつでも削除できます。
...
## 読み上げ（Cloud Text-to-Speech）の設定

添削画面の「修正版」「模範解答」の読み上げは、Gemini とは**別のAPI**である Google Cloud Text-to-Speech（WaveNet音声）を使います。使う前に Google Cloud 側で1度だけ設定が必要です。

1. [Google Cloud Console](https://console.cloud.google.com/) にアクセスします。AI Studio でAPIキーを作った場合、そのキーが属するプロジェクトが既にあります（AI Studio のAPIキー画面でプロジェクト名を確認できます）。
2. 対象プロジェクトを選び、「APIとサービス」→「ライブラリ」から **Cloud Text-to-Speech API** を検索して「有効にする」を押します。
3. 「お支払い」から**課金アカウントを紐付け**ます（無料枠の範囲でも紐付けは必要です）。WaveNet 音声は月100万文字まで無料で、教材の平均文字数（英語64文字・中国語10文字）なら月1.5万回ほど読める計算です。
4. 「APIとサービス」→「認証情報」→「認証情報を作成」→「APIキー」でキーを発行します。キーの制限で「APIの制限」を選び、Cloud Text-to-Speech API を許可してください。
5. アプリの設定画面「読み上げ（Cloud TTS）APIキー」欄に貼り付けて保存します。

**Gemini APIキーと同じプロジェクトのキーで Cloud Text-to-Speech API を有効化した場合、5番は不要です**（読み上げ用のキーが未登録なら Gemini のキーを流用します）。ただし AI Studio が発行するキーは Generative Language API のみに制限されていることがあり、その場合は403エラーになるので、上記の手順で読み上げ用のキーを別途登録してください。

読み上げに失敗すると、原因に応じた日本語メッセージがスナックバーで表示されます（API未有効化・キーの制限が原因の場合は、有効化を案内する文言が出ます）。
...
| 読み上げ（Cloud TTS）APIキー | Cloud Text-to-Speech 用のキー。**任意**で、未入力なら Gemini APIキーを流用します |
| モデル・音声認識（表示のみ） | 使用モデルは `gemini-3.8-flash` に固定。音声認識は録音をGeminiに送信して文字起こしする方式のみ（端末の音声認識は廃止）。読み上げは Cloud TTS の WaveNet 音声に固定 |
